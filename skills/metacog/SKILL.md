...
**THE ENVOY EXTREME** — Use when cross-model robustness matters or the target generator's response to register-shift is unknown. Three hard-extreme cross-domain author-becomes (cosmologists/world-builders at Sun Ra/Octavia Butler/Hilma af Klint scale, NOT mild-academic-essayists at Carson/Knuth scale) plus fork plus ritual. No register-shift -- the cross-model probe found register-shifts are generator-specific (catastrophic on gpt-5.5; lift-y on Sonnet) but cross-domain author-becomes transfer cleanly across all three models. Calibrated cross-model numbers: codex N=36 +0.283/0.236; Opus N=30 +0.379/0.310; Sonnet N=70 +0.190/0.257. Cross-model winner -- the only stratagem that survives the codex generator's response to tool-call conditioning above noise.
